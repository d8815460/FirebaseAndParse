...
class FUIAuthViewController: UITableViewController {
    ...
    func updateUI(auth: Auth, user: User?) {
        if let user = self.auth?.currentUser {
            ...
            self.loginParseMethod(auth: auth, user: user)
        } else {
    ...
    func loginParseMethod(auth:Auth, user:User?) {
        let pUser = PFUser()
        if (auth.currentUser?.isAnonymous != false) {
            
        } else {
            if user?.displayName != nil { pUser.username = user?.displayName }
            else if user?.email != nil { pUser.username = user?.email }
            else if user?.phoneNumber != nil {pUser.username = user?.phoneNumber }
            else { pUser.username = user?.uid }
            pUser.password = user?.uid
            if let photourl = user?.photoURL {
                pUser.setObject("\(photourl)" , forKey: "photoURL")
            }
            pUser.setObject(user?.phoneNumber ?? "", forKey: "phoneNumber")
            pUser.setObject(user?.email ?? "", forKey: "email")
            pUser.setObject(getAllAccessTokens(), forKey: "accessToken")
            pUser.setObject(getAllIdTokens(), forKey: "idToken")
            pUser.setObject(user?.uid as Any, forKey: "uid") // 等確認這個值永遠不會改變，再移除
            pUser.signUpInBackground { (succeeded, error) in
                if let error = error {
                    print("error:\(error.localizedDescription)") // Account already exists for this username
                } else {
                    PFUser.logInWithUsername(inBackground: pUser.username!, password: (user?.uid)!, block: { (cUser, error) in
                        print("current User = \(String(describing: cUser))")
                    })
                }
            }
        }
